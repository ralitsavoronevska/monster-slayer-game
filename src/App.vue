...
<template>
  <main :class="isDark ? 'dark' : ''" class="font-display">
    <div
      class="min-h-screen bg-linear-to-br from-indigo-100 to-pink-100 dark:from-gray-900 dark:to-gray-800 p-4"
    >
      <!-- CENTERED CONTAINER -->
      <div class="max-w-sm sm:max-w-md md:max-w-xl xl:max-w-6xl mx-auto">
        ...
        <div class="text-center mb-4">
          <span
            class="inline-flex items-center gap-3 px-8 py-3 bg-linear-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white font-bold rounded-lg shadow-lg transition transform hover:scale-105 cursor-default"
          >
            🏆 Best: {{ store.highScore }}
            {{ store.highScore === 1 ? "round" : "rounds" }}
          </span>
        </div>
        ...
        <!-- GAME TITLE -->
        <h1
          class="text-4xl font-bold text-center mb-6 text-gray-800 dark:text-gray-100"
        >
          Monster Slayer
        </h1>
        ...
        <HealthBar
          label="You"
          :percentage="store.playerBarStyles.width"
          color="bg-emerald-500"
        />
        <HealthBar
          label="Monster"
          :percentage="store.monsterBarStyles.width"
          color="bg-red-600"
        />
        ...
        <div class="flex flex-wrap gap-3 justify-center mt-8">
          ...
          <ActionButton
            label="Heal"
            variant="heal"
            @click="store.healPlayer()"
          />
          <ActionButton
            label="Surrender"
            variant="surrender"
            @click="store.surrender()"
          />
        </div>

        <!-- SPECIAL ATTACK -->
        <p
          v-if="!store.mayUseSpecialAttack && store.currentRound > 0"
          class="text-center mt-2 text-sm text-purple-600"
        >
